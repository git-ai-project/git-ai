...
fn test_cherry_pick_single_commit_preserves_ai_attribution() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "lib.rs"]).unwrap();
...
fn test_cherry_pick_range_preserves_attribution_across_commits() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "app.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();
    let main_branch = repo.current_branch();
...
fn test_reset_soft_recommit_preserves_ai_attribution() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "module.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();

    // AI commit 1
    repo.git_ai(&["checkpoint", "human", "module.rs"]).unwrap();
    fs::write(&file_path, "// module\nfn alpha() {}\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_ai", "module.rs"]).unwrap();
...
fn test_reset_mixed_then_selective_stage_and_commit() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "utils.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();
...
fn test_git_mv_preserves_attribution() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "old_name.rs"]).unwrap();
    repo.stage_all_and_commit("initial placeholder").unwrap();

    // AI rewrites the file
    repo.git_ai(&["checkpoint", "human", "old_name.rs"]).unwrap();
    fs::write(&file_path, "fn ai_code() {}\nfn human_code() {}\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_ai", "old_name.rs"]).unwrap();
...
fn test_two_ai_agents_same_file_one_commit() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "shared.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();

    // Agent 1 adds lines
    repo.git_ai(&["checkpoint", "human", "shared.rs"]).unwrap();
    fs::write(&file_path, "// shared module\nfn from_cursor() {}\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_ai", "shared.rs"]).unwrap();
    ...
    repo.stage_all_and_commit("both agents contributed").unwrap();
...
fn test_ai_edit_then_human_edit_same_file_one_commit() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "mixed.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();
...
fn test_revert_ai_commit_removes_ai_lines() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "feature.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();

    // AI adds a feature
    repo.git_ai(&["checkpoint", "human", "feature.rs"]).unwrap();
    fs::write(&file_path, "fn base() {}\nfn ai_feature() {}\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_ai", "feature.rs"]).unwrap();
...
fn test_fixup_autosquash_preserves_combined_attribution() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "impl.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();
    ...
    fs::write(&file_path, "// implementation\nfn process() {\n    let data = fetch();\n    transform(data)\n}\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_ai", "impl.rs"]).unwrap();
    repo.stage_all_and_commit(&format!("fixup! AI: add process stub")).unwrap();
...
fn test_commit_with_only_deletions_preserves_remaining_attribution() {
    ...
    repo.git_ai(&["checkpoint", "mock_ai", "shrink.rs"]).unwrap();
    repo.stage_all_and_commit("AI writes functions").unwrap();

    // Human deletes the middle function
    fs::write(&file_path, "fn keep() {}\nfn also_keep() {}\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_known_human", "shrink.rs"]).unwrap();
...
fn test_empty_commit_produces_no_note() {
    ...
    repo.git_ai(&["checkpoint", "mock_ai", "stable.rs"]).unwrap();
    repo.stage_all_and_commit("initial").unwrap();

    // Empty commit
    repo.git(&["commit", "--allow-empty", "-m", "empty"]).unwrap();
...
fn test_large_file_small_ai_edit() {
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "big.rs"]).unwrap();
    ...
    let ai_line = blame_output.lines().find(|l| l.contains("ai_edited_line")).unwrap();
    assert!(
