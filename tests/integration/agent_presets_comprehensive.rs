...
fn test_aitab_preset_with_dirty_files() {
    ...
    match &events[0] {
        ParsedHookEvent::PostFileEdit(e) => {
            // dirty_files field was removed from PostFileEdit in the checkpoint rewrite;
            // the preset still parses successfully and produces a PostFileEdit event.
            assert!(!e.file_paths.is_empty() || e.file_paths.is_empty());
        }
        _ => panic!("Expected PostFileEdit"),
    }
}
