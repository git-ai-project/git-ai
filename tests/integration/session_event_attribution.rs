...
use git_ai::metrics::{
    CheckpointValues, EventAttributes, MetricEvent, PosEncoded, SessionEventValues,
};
use serde_json::json;
...
fn insert_ai_checkpoint_event(
    ...
    let tool = if kind == "ai_tab" { "cursor" } else { "codex" };
    let session_id = generate_session_id(external_session_id, tool);
    let values = CheckpointValues::new()
        .checkpoint_ts(u64::from(event_ts))
        .kind(kind)
        .file_path(file_path)
        .lines_added(0)
        .lines_deleted(0)
        ...
        .external_tool_use_id(format!("tool-use-{external_session_id}"));
    let mut attrs = EventAttributes::with_version("test")
        .tool(tool)
        .model("gpt-5")
        .external_session_id(external_session_id)
        .session_id(&session_id)
        .trace_id(format!("trace-{external_session_id}"));
    if let Some(repo_url) = repo_url {
        attrs = attrs.repo_url(repo_url);
    }
    if let Some(base_commit) = base_commit {
        attrs = attrs.base_commit_sha(base_commit);
    }
    let event = MetricEvent::from_values_with_timestamp(values, attrs.to_sparse(), Some(event_ts));
    let event_json = serde_json::to_string(&event).expect("metric event should serialize");
    ...
    let mut db = MetricsDatabase::open_at_path(Path::new(db_path))
        .expect("metrics db should open at isolated path");
    db.insert_events(&[event_json])
        .expect("AI checkpoint event should insert");
...
fn assert_ai_checkpoint_mtime_recovery_for_kind(kind: &str, file_name: &str) {
    ...
    fs::write(&file_path, format!("generated by {kind}\n")).unwrap();
    let recovered_session_id = insert_ai_checkpoint_event(
        &metrics_db_path,
        file_mtime_secs(&file_path),
        kind,
        file_name,
        &format!("external-{kind}-session"),
        None,
        Some(base_commit.trim()),
    );

    let commit = repo
        .stage_all_and_commit(&format!("Recover {kind} attribution"))
        .expect("commit should succeed");

    let mut file = repo.filename(file_name);
    file.assert_committed_lines(lines![format!("generated by {kind}").as_str().ai()]);
...
#[test]
fn test_ai_checkpoint_mtime_recovery_supports_ai_agent() {
    assert_ai_checkpoint_mtime_recovery_for_kind("ai_agent", "agent.txt");
}

#[test]
fn test_ai_checkpoint_mtime_recovery_supports_ai_tab() {
    assert_ai_checkpoint_mtime_recovery_for_kind("ai_tab", "tab.txt");
}

#[test]
fn test_ai_checkpoint_mtime_recovery_handles_same_repo_path_mismatch() {
    let (_metrics_db_dir, metrics_db_path) = isolated_metrics_db_path();
    let repo =
        TestRepo::new_with_daemon_env(&[("GIT_AI_TEST_METRICS_DB_PATH", metrics_db_path.as_str())]);
    let old_path = repo.path().join("old-name.txt");
    fs::write(&old_path, "existing line\n").unwrap();
    let initial = repo
        .stage_all_and_commit("Initial commit")
        .expect("initial commit should succeed");
    let mut old_file = repo.filename("old-name.txt");
    old_file.assert_committed_lines(lines!["existing line".unattributed_human()]);

    fs::write(&old_path, "existing line\nrenamed AI line\n").unwrap();
    let new_path = repo.path().join("new-name.txt");
    ...
    let recovered_session_id = insert_ai_checkpoint_event(
        ...
        "ai_agent",
        "old-name.txt",
        "external-rename-session",
        None,
        Some(&initial.commit_sha),
    );

    let commit = repo
        .stage_all_and_commit("Rename AI file")
        .expect("rename commit should succeed");
    let mut new_file = repo.filename("new-name.txt");
    new_file.assert_committed_lines(lines![
...
#[test]
fn test_ai_checkpoint_mtime_recovery_runs_after_bash() {
    let (_metrics_db_dir, metrics_db_path) = isolated_metrics_db_path();
    ...
    let file_path = repo.path().join("bash-wins.txt");
    fs::write(&file_path, "generated during bash\n").unwrap();
    let file_ts = file_mtime_secs(&file_path);
    let checkpoint_session_id = insert_ai_checkpoint_event(
        &metrics_db_path,
        file_ts,
        "ai_agent",
        "bash-wins.txt",
        "external-checkpoint-session",
        None,
        Some(base_commit.trim()),
    );
    let bash_session_id = insert_bash_call(
        ...
        file_ts,
        "external-bash-session",
        "tool-use-bash",
    );

    let commit = repo
        .stage_all_and_commit("Bash recovery wins")
        .expect("commit should succeed");
    let mut file = repo.filename("bash-wins.txt");
    file.assert_committed_lines(lines!["generated during bash".ai()]);
    assert!(
        commit
            .authorship_log
            .metadata
    ...
    assert!(
        !commit
            ...
            .metadata
            .sessions
            .contains_key(&checkpoint_session_id)
    );
}

#[test]
fn test_ai_checkpoint_mtime_recovery_preserves_known_human() {
    let (_metrics_db_dir, metrics_db_path) = isolated_metrics_db_path();
    let repo =
        TestRepo::new_with_daemon_env(&[("GIT_AI_TEST_METRICS_DB_PATH", metrics_db_path.as_str())]);
    repo.git(&["commit", "--allow-empty", "-m", "initial"])
    ...
    let file_path = repo.path().join("human-checkpoint.txt");
    fs::write(&file_path, "typed by a human\n").unwrap();
    repo.git_ai(&["checkpoint", "mock_known_human", "human-checkpoint.txt"])
        .expect("known-human checkpoint should succeed");
    let recovered_session_id = insert_ai_checkpoint_event(
        &metrics_db_path,
        file_mtime_secs(&file_path),
        "ai_agent",
        "human-checkpoint.txt",
        "external-nearby-ai-session",
        None,
        Some(base_commit.trim()),
    );
    ...
    let commit = repo
        ...
        .expect("commit should succeed");
    let mut file = repo.filename("human-checkpoint.txt");
