...
use git_ai::commands::checkpoint_agent::kilo_code_preset::KiloCodePreset;
use serde_json::json;
use std::fs;
use test_utils::fixture_path;

fn kilo_code_storage_fixture_path() -> std::path::PathBuf {
    fixture_path("kilo-code-storage")
}

fn kilo_code_sqlite_fixture_path() -> std::path::PathBuf {
    fixture_path("kilo-code-sqlite")
}

fn kilo_code_sqlite_empty_fixture_path() -> std::path::PathBuf {
    fixture_path("kilo-code-sqlite-empty")
}

#[test]
fn test_parse_kilo_code_storage_transcript() {
    let storage_path = kilo_code_storage_fixture_path();
    let session_id = "test-session-123";

    let (transcript, model) =
        KiloCodePreset::transcript_and_model_from_storage(&storage_path, session_id)
            .expect("Failed to parse Kilo Code storage");
...
#[test]
fn test_parse_kilo_code_sqlite_transcript() {
    let kilo_code_root = kilo_code_sqlite_fixture_path();
    let session_id = "test-session-123";

    let (transcript, model) =
        KiloCodePreset::transcript_and_model_from_storage(&kilo_code_root, session_id)
            .expect("Failed to parse Kilo Code sqlite storage");
...
#[test]
fn test_kilo_code_sqlite_takes_precedence_over_legacy_storage() {
    let temp_dir = tempfile::tempdir().unwrap();
    let kilo_code_root = temp_dir.path();

    let sqlite_db = kilo_code_sqlite_fixture_path().join("kilo.db");
    fs::copy(&sqlite_db, kilo_code_root.join("kilo.db")).unwrap();

    let legacy_storage = kilo_code_storage_fixture_path();
    copy_dir_all(&legacy_storage, &kilo_code_root.join("storage")).unwrap();

    let (transcript, model) =
        KiloCodePreset::transcript_and_model_from_storage(kilo_code_root, "test-session-123")
...
#[test]
fn test_kilo_code_sqlite_falls_back_to_legacy_storage_when_sqlite_empty() {
    let temp_dir = tempfile::tempdir().unwrap();
    let kilo_code_root = temp_dir.path();

    let sqlite_db = kilo_code_sqlite_empty_fixture_path().join("kilo.db");
    fs::copy(&sqlite_db, kilo_code_root.join("kilo.db")).unwrap();
...
#[test]
fn test_kilo_code_transcript_message_order() {
    let storage_path = kilo_code_storage_fixture_path();
    let session_id = "test-session-123";
...
#[test]
fn test_kilo_code_transcript_timestamps_are_rfc3339() {
    let storage_path = kilo_code_storage_fixture_path();
    let session_id = "test-session-123";
...
fn test_kilo_code_preset_pretooluse_returns_human_checkpoint() {
    let storage_path = kilo_code_storage_fixture_path();
    ...
    let result = KiloCodePreset
        .run(flags)
        .expect("Failed to run KiloCodePreset");

    // Clean up env var
    unsafe {
        std::env::remove_var("GIT_AI_KILO_CODE_STORAGE_PATH");
    }
...
#[test]
#[serial_test::serial] // Run serially to avoid env var conflicts with other tests
fn test_kilo_code_preset_posttooluse_returns_ai_checkpoint() {
    let storage_path = kilo_code_storage_fixture_path();
    ...
    assert_eq!(result.agent_id.tool, "kilo-code");
    assert_eq!(result.agent_id.id, "test-session-123");
    assert_eq!(result.agent_id.model, "anthropic/claude-sonnet-4-20250514");
}

#[test]
#[serial_test::serial] // Run serially to avoid env var conflicts with other tests
fn test_kilo_code_preset_stores_session_id_in_metadata() {
    let storage_path = kilo_code_storage_fixture_path();
...
#[test]
#[serial_test::serial] // Run serially to avoid env var conflicts with other tests
fn test_kilo_code_preset_sets_repo_working_dir() {
    let storage_path = kilo_code_storage_fixture_path();
...
#[test]
fn test_kilo_code_empty_session_returns_empty_transcript() {
...
#[test]
fn test_kilo_code_nonexistent_session_returns_empty_transcript() {
    let storage_path = kilo_code_storage_fixture_path();
...
#[test]
fn test_kilo_code_tool_use_only_from_assistant() {
    let storage_path = kilo_code_storage_fixture_path();
    let session_id = "test-session-123";
...
#[test]
#[serial_test::serial] // Run serially to avoid env var conflicts with other tests
fn test_kilo_code_e2e_checkpoint_and_commit() {
    ...
    let fixture_storage = kilo_code_storage_fixture_path();
    ...
    repo.git_ai(&["checkpoint", "kilo-code", "--hook-input", &pre_hook_input])
        .unwrap();
    ...
    // Run AI checkpoint
    repo.git_ai(&["checkpoint", "kilo-code", "--hook-input", &post_hook_input])
        .unwrap();
    ...
    assert_eq!(
        prompt_record.agent_id.tool, "kilo-code",
        "Agent tool should be kilo-code"
    );
    assert_eq!(
        prompt_record.agent_id.model, "anthropic/claude-sonnet-4-20250514",
...
reuse_tests_in_worktree!(
    test_parse_kilo_code_storage_transcript,
    test_parse_kilo_code_sqlite_transcript,
    test_kilo_code_sqlite_takes_precedence_over_legacy_storage,
    test_kilo_code_sqlite_falls_back_to_legacy_storage_when_sqlite_empty,
    test_kilo_code_transcript_message_order,
    test_kilo_code_transcript_timestamps_are_rfc3339,
    test_kilo_code_empty_session_returns_empty_transcript,
    test_kilo_code_nonexistent_session_returns_empty_transcript,
    test_kilo_code_tool_use_only_from_assistant,
);
