...
use repos::test_repo::{default_branchname, NewCommit, TestRepo, WorktreeRepo};
...
trait RepoOps {
    ...
    fn git_from_working_dir(&self, working_dir: &Path, args: &[&str]) -> Result<String, String>;
    fn commit(&self, message: &str) -> Result<NewCommit, String>;
    fn stage_all_and_commit(&self, message: &str) -> Result<NewCommit, String>;
}
...
impl RepoOps for TestRepo {
    ...
    fn git_from_working_dir(&self, working_dir: &Path, args: &[&str]) -> Result<String, String> {
        self.git_from_working_dir(working_dir, args)
    }
    fn commit(&self, message: &str) -> Result<NewCommit, String> {
        self.commit(message)
    }
...
fn status_summary(repo: &impl RepoOps) -> (CommitStats, Vec<(u32, u32, bool, String)>) {
    ...
    let checkpoints = parsed
        .checkpoints
        .iter()
        .map(|cp| (cp.additions, cp.deletions, cp.is_human, cp.tool_model.clone()))
        .collect::<Vec<_>>();
...
fn worktree_commondir(worktree: &WorktreeRepo) -> PathBuf {
    ...
    let commondir_contents =
        fs::read_to_string(&commondir_path).expect("commondir should exist");
...
fn test_worktree_diff_parity() {
    ...
    let base_diff = base_repo.git_ai(&["diff", &base_commit.commit_sha]).unwrap();
...
fn test_group_files_by_repository_with_worktree() {
    ...
    let (repos, orphans) = group_files_by_repository(
...
fn test_worktree_config_overrides_global_config() {
    ...
    base_repo
        .git_with_env(&["config", "--global", "user.name", "Global"], &[("HOME", home_str)], None)
