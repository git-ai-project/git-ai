...
impl KeyringBackend {
    ...
    pub fn is_available(service_name: &str) -> bool {
        let test_entry = match keyring_crate::Entry::new(service_name, "test-availability") {
            Ok(entry) => entry,
            Err(_) => return false,
        };
...
impl CredentialBackend for KeyringBackend {
    fn store(&self, value: &str) -> Result<(), String> {
        let entry = keyring_crate::Entry::new(&self.service_name, &self.username)
    ...
    fn clear(&self) -> Result<(), String> {
        ...
        match entry.delete_credential() {
            Ok(()) => Ok(()),
            Err(keyring_crate::Error::NoEntry) => Ok(()), // Already cleared
