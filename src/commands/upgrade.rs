...
fn persist_update_state(
    ...
    persist_update_state_with_next_check(channel, release, upgrade_jitter_seconds, None);
}
...
pub(crate) fn clear_cached_update_state() {
    ...
    persist_update_state(channel, None, config.upgrade_jitter_seconds());
}
...
pub fn maybe_schedule_background_update_check() {
    ...
    if !should_check_for_updates(channel, cache.as_ref(), upgrade_jitter_seconds) {
        return;
    }
...
pub fn check_and_install_update_if_available() -> Result<DaemonUpdateCheckResult, String> {
    ...
    persist_update_state(channel, None, upgrade_jitter_seconds);
...
mod tests {
    ...
    fn test_should_check_for_updates_respects_interval() {
        ...
        assert!(should_check_for_updates(UpdateChannel::Latest, None, 0));
    }
    ...
    fn test_check_for_update_available_no_cache_newer_version() {
        ...
        persist_update_state(UpdateChannel::Latest, Some(&release), 0);
        let cache = read_update_cache().unwrap();
        assert!(cache.update_available());
