...
// Kilo Code plugin content (TypeScript), embedded from the source file
const KILO_CODE_PLUGIN_CONTENT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/agent-support/kilo-code/git-ai.ts"
));

pub struct KiloCodeInstaller;
...
impl KiloCodeInstaller {
    ...
    fn generate_plugin_content(binary_path: &Path) -> String {
        ...
        KILO_CODE_PLUGIN_CONTENT.replace("__GIT_AI_BINARY_PATH__", &path_str)
    }
}

impl HookInstaller for KiloCodeInstaller {
    fn name(&self) -> &str {
        "Kilo Code"
    }
...
mod tests {
    ...
    #[test]
    fn test_kilo_code_install_plugin_creates_file_from_scratch() {
        ...
        // Uses the kilo-code preset with session_id-based hook input and absolute path
    ...
    #[test]
    fn test_kilo_code_plugin_content_is_valid_typescript() {
        let content = KILO_CODE_PLUGIN_CONTENT;
    ...
    #[test]
    fn test_kilo_code_plugin_placeholder_substitution() {
        let binary_path = create_test_binary_path();
        let content = KiloCodeInstaller::generate_plugin_content(&binary_path);
        ...
        assert!(content.contains("${GIT_AI_BIN} checkpoint kilo-code"));
    }

    #[test]
    fn test_kilo_code_plugin_skips_if_already_exists() {
        let (_temp_dir, plugin_path) = setup_test_env();
        let binary_path = create_test_binary_path();
    ...
    #[test]
    fn test_kilo_code_plugin_updates_outdated_content() {
        let (_temp_dir, plugin_path) = setup_test_env();
        let binary_path = create_test_binary_path();
    ...
    #[test]
    fn test_kilo_code_plugin_windows_path_escaping() {
    ...
    #[test]
    fn test_kilo_code_plugin_handles_empty_directory() {
        let temp_dir = TempDir::new().unwrap();
        let binary_path = create_test_binary_path();
        let plugin_path = temp_dir
            .path()
