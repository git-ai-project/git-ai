...
impl PersistedWorkingLog {
    ...
    pub fn write_initial_attributions(
        ...
        // Normalize all file paths to Git format (forward slashes) and filter out empty attributions
        let filtered: HashMap<String, Vec<LineAttribution>> = attributions
            .into_iter()
            .filter(|(_, attrs)| !attrs.is_empty())
            .map(|(path, attrs)| (crate::utils::normalize_path_to_git_format(&path), attrs))
            .collect();
    ...
    pub fn read_initial_attributions(&self) -> InitialAttributions {
        ...
        match fs::read_to_string(&initial_file) {
            Ok(content) => match serde_json::from_str::<InitialAttributions>(&content) {
                Ok(mut initial_data) => {
                    // Normalize all file paths to Git format (for backwards compatibility)
                    initial_data.files = initial_data
                        .files
                        .into_iter()
                        .map(|(path, attrs)| {
                            (crate::utils::normalize_path_to_git_format(&path), attrs)
                        })
                        .collect();
                    initial_data
                }
