...
impl ActorDaemonCoordinator {
    ...
    async fn drain_ready_family_sequencer_entries_locked(
        ...
        let mut ready_command_count = 0usize;
        let mut checkpoint_count = 0usize;
        {
            ...
            while let Some(first_entry) = state.entries.first_entry() {
                ...
                match entry {
                    ...
                    other => {
                        match other.retention_kind() {
                            Some(SequencerRetentionKind::ReadyCommand) => {
                                ready_command_count = ready_command_count.saturating_add(1);
                            }
                            Some(SequencerRetentionKind::Checkpoint) => {
                                checkpoint_count = checkpoint_count.saturating_add(1);
                            }
                            Some(SequencerRetentionKind::PendingRoot) | None => {}
                        }
        ...
        let _retention_guard = SequencerReadyRetentionGuard {
            ready_commands: &self.retained_sequencer_ready_commands,
            checkpoints: &self.retained_sequencer_checkpoints,
            ready_command_count,
            checkpoint_count,
        };
