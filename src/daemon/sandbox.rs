...
fn detect_security_sandbox_env() -> Option<(&'static str, String)> {
    for (sandbox, keys) in [
        ("Cursor", &["CURSOR_SANDBOX"][..]),
        (
            "git-ai configured",
            &["GIT_AI_SECURITY_SANDBOX", "GIT_AI_AGENT_SECURITY_SANDBOX"][..],
        ),
    ] {
        if let Some(key) = keys.iter().find(|key| std::env::var_os(key).is_some()) {
            return Some((sandbox, (*key).to_string()));
        }
    }
...
mod tests {
    ...
    fn ignores_cloud_agent_and_undocumented_backend_signals() {
        ...
        let _codex_sandbox = ScopedEnvVar::set("CODEX_SANDBOX", "seatbelt");
