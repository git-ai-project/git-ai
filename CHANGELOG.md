...
- **Kiro (kiro-cli) preset and hook installer**: First-class support for AWS Kiro's CLI hook protocol ([#584](https://github.com/git-ai-project/git-ai/issues/584))
  - New `kiro` preset (`presets/kiro.rs`) — pure parser for Kiro's hook payloads (`preToolUse`/`postToolUse` for `fs_write`/`write`, `execute_bash`/`shell`)
  - New `KiroInstaller` (`mdm/agents/kiro.rs`) — installs hook entries into a per-agent JSON file at `~/.kiro/agents/default.json`. Idempotent, preserves user-defined hooks and agent settings.
  - Strict event handling: lifecycle events (`agentSpawn`, `userPromptSubmit`, `stop`) and PascalCase event names (Claude-shaped) are explicitly rejected rather than silently misrouted
  - Tool classification: `fs_write`/`write` → file edits, `execute_bash`/`shell` → bash; documented read-only/external tools (`fs_read`/`read`, `use_aws`/`aws`, `@server/*` MCP) are skipped
  - Targets the Kiro CLI (`kiro-cli`) only — the IDE GUI hook system has an undocumented payload schema and is intentionally out of scope for v1
  - Transcript reading deferred: Kiro stores conversation history in an undocumented SQLite database under `~/.kiro/`; a SQLite-based reader can land in a follow-up once the schema is documented upstream
