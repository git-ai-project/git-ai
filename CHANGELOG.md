...
- **Hermes (NousResearch hermes-agent) preset and hook installer**: First-class support for hermes-agent's shell-hook protocol ([#1136](https://github.com/git-ai-project/git-ai/issues/1136))
  - New `hermes` preset (`presets/hermes.rs`) — pure parser for Hermes' shell-hook payloads (`pre_tool_call`/`post_tool_call` for `write_file`, `patch`, `terminal`)
  - New `HermesInstaller` (`mdm/agents/hermes.rs`) — installs `hooks:` block entries into `~/.hermes/config.yaml` with the `".*"` matcher; idempotent, preserves user-defined hooks; sets `hooks_auto_accept: true` only when the user hasn't explicitly chosen
  - Strict event handling: lifecycle events (`pre_llm_call`, `on_session_start`, `subagent_stop`, etc.) are explicitly rejected rather than silently misrouted
  - Tool classification: `write_file`/`patch` → file edits, `terminal` → bash; documented read-only tools (`read_file`, `search_files`) are skipped
  - New `serde_yml` dependency for parsing `~/.hermes/config.yaml` (Hermes is the first agent that uses YAML config)
