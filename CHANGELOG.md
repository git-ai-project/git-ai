...
- **Kimi Code (Moonshot AI) preset and hook installer**: First-class support for Moonshot AI's `kimi-cli` coding assistant ([#639](https://github.com/git-ai-project/git-ai/issues/639))
  - New `kimi-code` preset (`presets/kimi_code.rs`) — pure parser for kimi-cli's hook payloads (`PreToolUse`/`PostToolUse` for `WriteFile`, `StrReplaceFile`, `Shell`)
  - New `KimiCodeInstaller` (`mdm/agents/kimi_code.rs`) — installs `[[hooks]]` entries into `~/.kimi/config.toml`; idempotent, preserves user-defined hooks
  - Strict event handling: lifecycle events (`SessionStart`, `Stop`, `SubagentStart`, `PostToolUseFailure`, etc.) are explicitly rejected rather than silently misrouted
  - Tool classification: `WriteFile`/`StrReplaceFile` → file edits, `Shell` → bash; `ReadFile`/`Grep`/`Glob`/`SearchWeb`/`FetchURL`/`LaborMarket` are skipped
