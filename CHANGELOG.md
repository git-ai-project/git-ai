...
- **Cline (cline.bot VS Code extension) preset and hook installer**: First-class support for the Cline IDE plugin's hook protocol ([#538](https://github.com/git-ai-project/git-ai/issues/538))
  - New `cline` preset (`presets/cline.rs`) — pure parser for Cline's hook payloads (`PreToolUse`/`PostToolUse` for `write_to_file`, `replace_in_file`, `execute_command`)
  - New `ClineInstaller` (`mdm/agents/cline.rs`) — drops extensionless executable shell scripts (`PreToolUse`, `PostToolUse`) into `~/Documents/Cline/Hooks/` per Cline's filename-as-registration convention. Sets the executable bit on Unix; uses `.ps1` on Windows. Idempotent; refuses to overwrite user-owned scripts at the same paths.
  - Strict event handling: lifecycle events (`TaskStart`, `TaskResume`, `TaskCancel`, `TaskComplete`, `UserPromptSubmit`, `PreCompact`) are explicitly rejected rather than silently misrouted
  - Tool classification: `write_to_file`/`replace_in_file` → file edits, `execute_command` → bash; documented read-only/non-mutating tools (`read_file`, `search_files`, `list_files`, `list_code_definition_names`, `browser_action`, `use_mcp_tool`, `access_mcp_resource`, `ask_followup_question`, `attempt_completion`, `new_task`) are skipped
  ...
  - Model extraction: `model.slug` is read into `agent_id.model`; tolerates the older bare-string shape
