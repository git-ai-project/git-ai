...
- **Augment Code (Auggie CLI) preset and hook installer**: First-class support for Augment Code, autodetecting both the v1 (`auggie`) and v2 (`auggie-v2` / `cosmos-agent`) hook payload shapes from a single preset (closes #1071)
  - New `augment` preset (`presets/augment.rs`) parses `PreToolUse`/`PostToolUse` for `save-file`, `str-replace-editor`, `remove-files`, `apply_patch`, `launch-process` (v1) and `write`, `edit`, `terminal`, `bash` (v2); read-only tools and lifecycle events are a silent no-op so the catch-all hook never surfaces spurious warnings; a v2 payload may also supply a `conversation_id` (honoured verbatim when present) for per-conversation session attribution, falling back to the existing workspace-derived id when absent
  - New `AugmentInstaller` (`mdm/agents/augment.rs`) writes a catch-all `".*"` matcher into `~/.augment/settings.json`, idempotently, preserving any user-defined hooks; install also sweeps owned git-ai commands out of any other matcher block (legacy hand-written matchers, duplicate catch-all blocks), so a checkpoint never fires twice
